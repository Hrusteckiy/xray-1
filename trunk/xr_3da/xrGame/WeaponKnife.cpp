...
void CWeaponKnife::Load	(LPCSTR section)
{
	...
	if(pSettings->line_exist(*hud_sect, "anim_idle_sprint"))
		animGet(mhud_sprint, pSettings->r_string(*hud_sect, "anim_idle_sprint"));

	if (pSettings->line_exist(*hud_sect, "anim_idle_moving"))
		animGet(mhud_moving, pSettings->r_string(*hud_sect, "anim_idle_moving"));
}
...
void CWeaponKnife::PlayAnimIdle()
{
	...
	if (actor)
		...
		{
			m_pHUD->animPlay(random_anim(mhud_sprint), TRUE, this, GetState());
			return;
		}
		else if (actor->is_actor_running())
		{
			m_pHUD->animPlay(random_anim(mhud_moving), TRUE, this, GetState());
			return;
		}
